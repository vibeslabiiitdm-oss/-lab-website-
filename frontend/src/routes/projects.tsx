    ...
    <div className="mx-auto max-w-7xl px-6 py-14">
      ...
      <Reveal>
        ...
        <p className="mt-3 max-w-2xl text-muted-foreground">
          ...
          processing - click a project to explore its details, publications, and photos.
        </p>
      </Reveal>
      ...
      <div className="mt-10 grid gap-8">
        {projects.map((proj, i) => (
          <Reveal key={proj.id} delay={i * 50}>
            <article
              className="group relative grid md:grid-cols-[1.05fr_1fr] gap-0 rounded-3xl border border-border/60 glass overflow-hidden transition cursor-pointer hover:border-primary/40 hover:shadow-lg hover:shadow-primary/5"
            ...
            >
              ...
              <div className="p-6 md:p-8">
                ...
                <div className="mt-4">
                  <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Purpose</div>
                  <p className="mt-1 text-sm text-foreground/90 leading-relaxed">{proj.purpose}</p>
                </div>

                <div className="mt-3">
                  <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Approach</div>
                  <p className="mt-1 text-sm text-muted-foreground leading-relaxed">{proj.description}</p>
                </div>

                <div className="mt-4">
                  <div className="text-[10px] uppercase tracking-[0.18em] text-muted-foreground">Results</div>
                  <ul className="mt-1.5 space-y-1.5">
                    ...
                    {proj.results.length > 3 && (
                      <li className="text-xs text-muted-foreground pl-5">
                        +{proj.results.length - 3} more on the detail page
                      </li>
                    )}
                  </ul>
                </div>
                ...
                <div className="mt-5 flex flex-wrap gap-1.5">
                  {proj.tech.map((t) => (
                    <span key={t} className="text-[10px] px-2 py-0.5 rounded-full bg-accent/10 border border-accent/25 text-accent">
                      {t}
                    </span>
                  ))}
                </div>
                ...
                <div className="mt-4 inline-flex items-center gap-1 text-xs text-primary/70 font-medium group-hover:text-primary transition">
                  View details, publications &amp; photos <ArrowRight size={12} />
                </div>
              </div>
            </article>
          </Reveal>
