...
export const mkAwards = (seed, n) => Array.from({ length: n }).map((_, i) => ({
    id: `a-${seed}-${i}`,
    title: [
        "Best Paper Award",
...
export const mkConfs = (seed, n) => Array.from({ length: n }).map((_, i) => ({
    id: `c-${seed}-${i}`,
...
export let guide = {
    ...
    publications: [
        {
            id: "pub-pat-1",
            title: "Position and time localization of occlusion between two objects using a static camera",
        ...
        {
            id: "pub-sm-1",
            title: "Acquisition and corpus description of a constrained lip database captured from handheld devices",
...
export let scholars = [
    {
        ...
        publications: [
            {
                id: "pub-sukesh-j1",
                title: "Costaa YOLO: Convolutional Swin Transformer with Attention and Anchor Optimization for Robust Pedestrian Detection",
            ...
            {
                id: "pub-sukesh-j2",
                title: "MECSA: Multi-scale Enhanced Channel and Spatial Attention for Robust Pedestrian Detection",
            ...
            {
                id: "pub-sukesh-j4",
                title: "DECA-YOLO: Dual-Pooling Efficient Channel Attention for Robust Pedestrian Detection",
        ...
        researchProject: {
            title: "Robust Lightweight Pedestrian Detection Algorithm for Autonomous Mobile Robots (AMCR)",
            abstract: "Developing high-efficiency, lightweight deep learning algorithms optimized for real-time pedestrian detection on edge computing devices and autonomous mobile robots (AMCR). The focus is on handling low-light, occlusion, and multi-scale detection challenges. As part of this research, we created the CamPed (Campus Pedestrians) and Enriched CamPed datasets, captured within the IIITDM Kancheepuram campus, containing 1516 videos converted to 100K annotated frames.",
            datasets: [
    ...
    },
    {
        ...
        researchProject: {
            ...
            abstract: "Developing navigation and tracking pipelines for AGVs in GPS-denied environments. Created a real-time 3-model pipeline leveraging YOLOv4, Deep-SORT, and a geometry-based ML model (PDM) for monocular object trajectory prediction.",
            datasets: ["TIHAAN AGV Dataset"],
            results: [
...
export let allPeople = [guide, ...scholars];
...
export let projects = [
    {
        ...
        description: "Funded by TiHAN-IIT Hyderabad with a financial outlay of 19.02 Lakhs for a duration of 18 Months.",
    ...
    },
    {
        ...
        status: "Ongoing",
        year: 2024,
        purpose: "Create a tamper-proof and robust identification system for cattle using multi-biometric traits (periocular, muzzle) alongside health monitoring.",
        description: "Funded by the Department of Science and Technology, Govt. of India with a financial outlay of 18.32 Lakhs for 2 Years. Status: Technical Acceptance.",
...
export function getPublicationsByDomain(domain) {
    const d = domain.toLowerCase();
    return allPeople.flatMap((p) => p.publications
        .filter((pub) => pub.domain.toLowerCase().includes(d) || d.includes(pub.domain.toLowerCase()))
        .map((pub) => ({ ...pub, author: p.name, authorId: p.id })));
}
export function getConferencesByDomain(_domain) {
    // Conferences aren't tagged by domain in mock data — return them all as related events.
    return allPeople.flatMap((p) => p.conferences.map((c) => ({ ...c, author: p.name, authorId: p.id })));
}
export let supervisedProjects = [
    {
        sno: 1,
...
const isSeeding = typeof globalThis.process !== "undefined" && globalThis.process.argv && globalThis.process.argv.some((arg) => arg.includes("seed") || arg.includes("check_db") || arg.includes("test_import"));
if (!isSeeding) {
    try {
